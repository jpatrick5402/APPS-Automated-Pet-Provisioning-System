...
void loop()
{
  ...
  if (is_DST(timeClient.getEpochTime()))
  {
  ...
  Serial.println(time);
  for (String t : times)
  { // loop through times and check if any of them are now
    if (t == time)
    {
      // add food until full
      ...
      if (counter >= CYCLE_LIMIT)
        send_email("Food Container is empty or obstruction detected, please remedy ASAP");
      ...
      // send email if refill needed
      if (counter >= CYCLE_LIMIT)
        send_email("Water Resivoir is empty or obstruction detected, please remedy ASAP");
